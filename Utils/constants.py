# FILE SUFFIXES
KERAS_SUFFIX = ".keras"
NUMPY_SUFFIX = ".npy"
PICKLE_SUFFIX = ".pkl"

# DATA  FOLDER
PL_AMP_PHASE_DATA_PATH = "/home/dani/DaniProjects/SAIL/PhotonicLanternProjects/Data/AmpPhaseReconstruction"

# ORIGINAL DATA FOLDERS
ORIGINAL_FOLDER_DATA_PATH = f"{PL_AMP_PHASE_DATA_PATH}/OriginalData"

ORIGINAL_FLUXES_FOLDER = f"{ORIGINAL_FOLDER_DATA_PATH}/plfluxes_simplepoly__pllabdata_20230625a_superK_slmcube_20230625_complsines-01sp_04"
ORIGINAL_FLUXES_FILE = f"{ORIGINAL_FLUXES_FOLDER}/all_fluxes{NUMPY_SUFFIX}"

ORIGINAL_SLM_FOLDER = f"{ORIGINAL_FOLDER_DATA_PATH}/slmcube_20230625_complsines-01sp_04_PSFWFs_file"
ORIGINAL_AMPLITUDE_FILENAME = "complexsine_pupamp.npy"
ORIGINAL_PHASE_FILENAME = "complexsine_pupphase.npy"

# PROCESSED DATA FOLDERS
PROCESSED_DATA_FOLDER = f"{PL_AMP_PHASE_DATA_PATH}/ProcessedData"

TRAIN_FOLDER = f"{PROCESSED_DATA_FOLDER}/TrainData"
VALIDATION_FOLDER = f"{PROCESSED_DATA_FOLDER}/ValidationData"
TEST_FOLDER = f"{PROCESSED_DATA_FOLDER}/TestData"
SCALERS_FOLDER = f"{PROCESSED_DATA_FOLDER}/Scalers"

# PROCESSED DATA FILENAMES
FC_FLUXES_FILENAME_PREFIX = f"fully_connected_fluxes"
CNN_FLUXES_FILENAME_PREFIX = f"convolutional_fluxes"
AUTOENCODER_FLUXES_FILENAME_PREFIX = f"autoencoder_fluxes"

FC_FLUX_SCALER_FILENAME = f"fc_flux_scaler{PICKLE_SUFFIX}"
CNN_FLUX_SCALER_FILENAME = f"convolutional_flux_scaler{PICKLE_SUFFIX}"
AUTOENCODER_FLUX_SCALER_FILENAME = f"autoencoder_flux_scaler{PICKLE_SUFFIX}"

FC_AMP_PHASE_FILENAME_PREFIX = f"fully_connected_amp_phase"
CNN_AMP_PHASE_FILENAME_PREFIX = f"convolutional_amp_phase"
AUTOENCODER_AMP_PHASE_FILENAME_PREFIX = "autoencoder_amp_phase"

FC_AMP_SCALER_FILENAME = f"fc_amp_scaler{PICKLE_SUFFIX}"
CNN_AMP_SCALER_FILENAME = f"convolutional_amp_scaler{PICKLE_SUFFIX}"
AUTOENCODER_AMP_SCALER_FILENAME = f"autoencoder_amp_scaler{PICKLE_SUFFIX}"

FC_PHASE_SCALER_FILENAME = f"fc_phase_scaler{PICKLE_SUFFIX}"
CNN_PHASE_SCALER_FILENAME = f"convolutional_phase_scaler{PICKLE_SUFFIX}"
AUTOENCODER_PHASE_SCALER_FILENAME = f"autoencoder_phase_scaler{PICKLE_SUFFIX}"

# TRAIN, VALIDATION AND TEST SUFFIXES FOR THE AMPLITUDE AND PHASE FILES
TRAIN_FILE_SUFFIXES = ["00", "01", "02", "03", "04", "05", "06"]
VALIDATION_FILE_SUFFIX = f"07"
TEST_FILE_SUFFIX = f"08"


# PROCESSED DATA PATHS
# - Fully connected architecture
TRAIN_FC_FLUXES_PATH = f"{TRAIN_FOLDER}/{FC_FLUXES_FILENAME_PREFIX}"
VALIDATION_FC_FLUXES_PATH = f"{VALIDATION_FOLDER}/{FC_FLUXES_FILENAME_PREFIX}{NUMPY_SUFFIX}"
TEST_FC_FLUXES_PATH = f"{TEST_FOLDER}/{FC_FLUXES_FILENAME_PREFIX}{NUMPY_SUFFIX}"

TRAIN_FC_AMP_PHASE_PATH = f"{TRAIN_FOLDER}/{FC_AMP_PHASE_FILENAME_PREFIX}"
VALIDATION_FC_AMP_PHASE_PATH = f"{VALIDATION_FOLDER}/{FC_AMP_PHASE_FILENAME_PREFIX}{NUMPY_SUFFIX}"
TEST_FC_AMP_PHASE_PATH = f"{TEST_FOLDER}/{FC_AMP_PHASE_FILENAME_PREFIX}{NUMPY_SUFFIX}"

FC_FLUX_SCALER_PATH = f"{SCALERS_FOLDER}/{FC_FLUX_SCALER_FILENAME}"
FC_AMP_SCALER_PATH = f"{SCALERS_FOLDER}/{FC_AMP_SCALER_FILENAME}"
FC_PHASE_SCALER_PATH = f"{SCALERS_FOLDER}/{FC_PHASE_SCALER_FILENAME}"

# - Convolutional architecture
TRAIN_CNN_FLUXES_PATH = f"{TRAIN_FOLDER}/{CNN_FLUXES_FILENAME_PREFIX}"
VALIDATION_CNN_FLUXES_PATH = f"{VALIDATION_FOLDER}/{CNN_FLUXES_FILENAME_PREFIX}{NUMPY_SUFFIX}"
TEST_CNN_FLUXES_PATH = f"{TEST_FOLDER}/{CNN_FLUXES_FILENAME_PREFIX}{NUMPY_SUFFIX}"

TRAIN_CNN_AMP_PHASE_PATH = f"{TRAIN_FOLDER}/{CNN_AMP_PHASE_FILENAME_PREFIX}"
VALIDATION_CNN_AMP_PHASE_PATH = f"{VALIDATION_FOLDER}/{CNN_AMP_PHASE_FILENAME_PREFIX}{NUMPY_SUFFIX}"
TEST_CNN_AMP_PHASE_PATH = f"{TEST_FOLDER}/{CNN_AMP_PHASE_FILENAME_PREFIX}{NUMPY_SUFFIX}"

CNN_FLUX_SCALER_PATH = f"{SCALERS_FOLDER}/{CNN_FLUX_SCALER_FILENAME}"
CNN_AMP_SCALER_PATH = f"{SCALERS_FOLDER}/{CNN_AMP_SCALER_FILENAME}"
CNN_PHASE_SCALER_PATH = f"{SCALERS_FOLDER}/{CNN_PHASE_SCALER_FILENAME}"

# - Autoencoder architecture
TRAIN_AUTOENCODER_FLUXES_PATH = f"{TRAIN_FOLDER}/{AUTOENCODER_FLUXES_FILENAME_PREFIX}"
VALIDATION_AUTOENCODER_FLUXES_PATH = f"{VALIDATION_FOLDER}/{AUTOENCODER_FLUXES_FILENAME_PREFIX}{NUMPY_SUFFIX}"
TEST_AUTOENCODER_FLUXES_PATH = f"{TEST_FOLDER}/{AUTOENCODER_FLUXES_FILENAME_PREFIX}{NUMPY_SUFFIX}"

TRAIN_AUTOENCODER_AMP_PHASE_PATH = f"{TRAIN_FOLDER}/{AUTOENCODER_AMP_PHASE_FILENAME_PREFIX}"
VALIDATION_AUTOENCODER_AMP_PHASE_PATH = f"{VALIDATION_FOLDER}/{AUTOENCODER_AMP_PHASE_FILENAME_PREFIX}{NUMPY_SUFFIX}"
TEST_AUTOENCODER_AMP_PHASE_PATH = f"{TEST_FOLDER}/{AUTOENCODER_AMP_PHASE_FILENAME_PREFIX}{NUMPY_SUFFIX}"

AUTOENCODER_FLUX_SCALER_PATH = f"{SCALERS_FOLDER}/{AUTOENCODER_FLUX_SCALER_FILENAME}"
AUTOENCODER_AMP_SCALER_PATH = f"{SCALERS_FOLDER}/{AUTOENCODER_AMP_SCALER_FILENAME}"
AUTOENCODER_PHASE_SCALER_PATH = f"{SCALERS_FOLDER}/{AUTOENCODER_PHASE_SCALER_FILENAME}"

# MODELS PATH
MODELS_FOLDER_PATH = "./Models"
MODELS_DESCRIPTION_FILE_PATH = "./models_descriptions.txt"

# INPUT AND OUTPUT SIZES
FC_INPUT_SHAPE = 1320
FC_OUTPUT_SHAPE = (2, 96, 96)

CNN_INPUT_SHAPE = (55, 24, 1)
CNN_OUTPUT_SHAPE = (96, 96, 2)

AUTOENCODER_INPUT_SHAPE = (56, 24, 1)

SUBFILE_SAMPLES = 10000


### PSF RECONSTRUCTION ###

PSF_DATA_PATH = "/home/dani/DaniProjects/SAIL/PhotonicLanternProjects/Data/PSFReconstruction"
COMPLEX_FIELDS_DATA_PATH = f"{PSF_DATA_PATH}/ComplexFields"
OUTPUT_FLUXES_DATA_PATH = f"{PSF_DATA_PATH}/OutputFluxes"

LANTERN_FIBER_FILENAME = f"/extractedvals_probeset_19LP__Good202107.npz"

TRAIN_DATA_FOLDER_NAME = "TrainData"
VALIDATION_DATA_FOLDER_NAME = "ValidationData"
TEST_DATA_FOLDER_NAME = "TestData"

COMPLEX_FIELDS_FILE_NAME = "complex_fields"
OUTPUT_FLUXES_FILE_NAME = "output_fluxes"

# Data folders for complex fields
TRAIN_COMPLEX_FIELDS_DATA_PATH = f"{COMPLEX_FIELDS_DATA_PATH}/{TRAIN_DATA_FOLDER_NAME}"
VALIDATION_COMPLEX_FIELDS_DATA_PATH = f"{COMPLEX_FIELDS_DATA_PATH}/{VALIDATION_DATA_FOLDER_NAME}"
TEST_COMPLEX_FIELDS_DATA_PATH = f"{COMPLEX_FIELDS_DATA_PATH}/{TEST_DATA_FOLDER_NAME}"

# Data folders for output fluxes
TRAIN_OUTPUT_FLUXES_DATA_PATH = f"{OUTPUT_FLUXES_DATA_PATH}/{TRAIN_DATA_FOLDER_NAME}"
VALIDATION_OUTPUT_FLUXES_DATA_PATH = f"{OUTPUT_FLUXES_DATA_PATH}/{VALIDATION_DATA_FOLDER_NAME}"
TEST_OUTPUT_FLUXES_DATA_PATH = f"{OUTPUT_FLUXES_DATA_PATH}/{TEST_DATA_FOLDER_NAME}"

# Data filenames for complex fields
TRAIN_COMPLEX_FIELDS_PREFIX = f"{TRAIN_COMPLEX_FIELDS_DATA_PATH}/{COMPLEX_FIELDS_FILE_NAME}"
VALIDATION_COMPLEX_FIELDS_PATH = f"{VALIDATION_COMPLEX_FIELDS_DATA_PATH}/{COMPLEX_FIELDS_FILE_NAME}{VALIDATION_FILE_SUFFIX}{NUMPY_SUFFIX}"
TEST_COMPLEX_FIELDS_PATH = f"{TEST_COMPLEX_FIELDS_DATA_PATH}/{COMPLEX_FIELDS_FILE_NAME}{TEST_FILE_SUFFIX}{NUMPY_SUFFIX}"

# Data filenames for output fluxes
TRAIN_OUTPUT_FLUXES_PREFIX = f"{TRAIN_OUTPUT_FLUXES_DATA_PATH}/{OUTPUT_FLUXES_FILE_NAME}"
VALIDATION_OUTPUT_FLUXES_PATH = f"{VALIDATION_OUTPUT_FLUXES_DATA_PATH}/{OUTPUT_FLUXES_FILE_NAME}{VALIDATION_FILE_SUFFIX}{NUMPY_SUFFIX}"
TEST_OUTPUT_FLUXES_PATH = f"{TEST_OUTPUT_FLUXES_DATA_PATH}/{OUTPUT_FLUXES_FILE_NAME}{TEST_FILE_SUFFIX}{NUMPY_SUFFIX}"


...
class DynamicFluxPSFDataset(Dataset):
	...
	def __init__(self, flux_file_paths, complex_fields_paths, do_shuffle):
		self.item_counter = 0
		self.next_file = 0
		self.complex_file_paths = complex_fields_paths
	...
	def load_next_file(self):
		...
		if self.next_file < len(self.file_paths):
			...
			if self.do_shuffle:
				...
				fluxes_arrays = fluxes_arrays[shuffled_indices]
                
			data = [fluxes_arrays, processed_complex_field_arrays]
			self.next_file += 1
...
def instantiate_flux_psf_dataloader(
	...
	if validation:
		...
		for file_number in TRAIN_FILE_SUFFIXES:
			...
			flux_subfile_path = f"{TRAIN_OUTPUT_FLUXES_PREFIX}{file_number}{NUMPY_SUFFIX}"
			...
			complex_field_file_paths.append(subfile_path)

	flux_psf_dynamic_dataset = DynamicCustomPSFDataset(file_paths, do_shuffle=do_shuffle)
